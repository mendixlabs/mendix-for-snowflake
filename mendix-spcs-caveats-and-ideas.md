...
**Future improvement (designed, not implemented):** A full recovery design was worked out during v2 feature planning but deliberately not built; the scope decision was to document the design here rather than implement it now. The design covers the five points below and can be implemented later without redoing the research.

**Recovery flow:** `GetCompoundToken.java:52-58` currently throws a plain `RuntimeException` on a cache miss. The design adds a new typed exception, `snowflakesso.CallerTokenExpiredException` (a sibling class of `CallerTokenCache.java`), thrown from that same cache-miss branch. Consumer microflows set Custom (without rollback) error handling on `GetCompoundToken`/`ExecuteQuery` and branch on `$latestError/ErrorType = 'snowflakesso.CallerTokenExpiredException'`. The true branch calls a shipped nanoflow that wraps a new JS action, `RedirectToHeaderSSO.js`.

**The JS action:** `RedirectToHeaderSSO.js` refreshes before it redirects. It first calls `fetch("/headersso/refresh")`. SPCS ingress re-injects a fresh `Sf-Context-Current-User-Token` on that request, `HeaderSSOHandler.java:207-236` re-caches it and returns 204, and the JS action returns true so the calling microflow retries the failed query once, with no visible interruption to the user. If the refresh call does not return 204 (the Mendix session itself is dead), the JS action falls back to a full page navigation to `/headersso/?cont=<pathname+search+hash>`, which re-runs the full header-SSO handshake and returns the user to where they were. The action keeps a module-scope loop guard so it will not fire a second redirect within roughly 30 seconds, and a shared in-flight promise so multiple queries failing at once coalesce into a single refresh call rather than racing.

**Why server-side self-heal is impossible:** Mendix gives UserActions no access to the request object, so a Java action cannot read or act on HTTP headers; only a `RequestHandler` can do that, and a `RequestHandler` cannot intercept calls under `/xas/`. The server also cannot call `/headersso/refresh` on its own behalf, because SPCS ingress only injects a fresh token into requests that arrive from the user's authenticated browser session, not into requests the server originates itself. That means the first query after expiry will always fail; there is no supported path to re-cache the token before that first failure happens, and recovery has to start in the browser.

**Packaging caveats:** `Update-SnowflakeSSOMpk.ps1` rebuilds the shared `.mpk` from source and syncs `package.xml` for new non-action files, so both new paths (`RedirectToHeaderSSO.js` and `CallerTokenExpiredException.java`) need to be added to its `$files` list. The new JS action and the nanoflow that wraps it are both stored in `project.mpr`, which the script never touches, so implementing this requires a one-time manual edit in Studio Pro to add the action and nanoflow, followed by a fresh baseline `.mpk` export before the update script can take over again.

**To verify at implementation time:** the exact `$latestError/ErrorType` string format on the Mendix version actually running (fully qualified Java class names can shift formatting across runtime versions), and whether the how-to guide for consumers needs a worked screenshot of the microflow's error-handling branch rather than just the description above.
